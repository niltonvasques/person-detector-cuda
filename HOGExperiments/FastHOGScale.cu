...
__host__ void DownscaleImage( FastHOG_::FastHOG *fHOG, int startScaleId, int endScaleId, int scaleId, float scale, 
							 ...
							 float1* resizedPaddedImageF1, float4* resizedPaddedImageF4)
{
	...
	fHOG->rPaddedWidth = iDivUpF(fHOG->hPaddedWidth , scale);
	fHOG->rPaddedHeight = iDivUpF(fHOG->hPaddedHeight , scale);

	hBlockSize = dim3(iDivUp(fHOG->rPaddedWidth, hThreadSize.x), iDivUp(fHOG->rPaddedHeight, hThreadSize.y));

	if (scaleId == startScaleId)
	{
		if (isAlocated)
			cutilSafeCall(cudaFreeArray(imageArray));
		cutilSafeCall(cudaMallocArray(&imageArray, &channelDescDownscale, fHOG->hPaddedWidth, fHOG->hPaddedHeight) );
		cutilSafeCall(cudaMemcpyToArray(imageArray, 0, 0, paddedRegisteredImage, sizeof(float4) * fHOG->hPaddedWidth * fHOG->hPaddedHeight, cudaMemcpyDeviceToDevice));
		isAlocated = true;
	}
	...
	if (useGrayscale)
	{
		cutilSafeCall(cudaMemset(resizedPaddedImageF1, 0, fHOG->hPaddedWidth * fHOG->hPaddedHeight * sizeof(float1)));
		resizeFastBicubic1<<<hBlockSize, hThreadSize>>>(resizedPaddedImageF1, paddedRegisteredImage, fHOG->rPaddedWidth, fHOG->rPaddedHeight, scale);
	}
	else
	{
		cutilSafeCall(cudaMemset(resizedPaddedImageF4, 0, fHOG->hPaddedWidth * fHOG->hPaddedHeight * sizeof(float4)));
		resizeFastBicubic4<<<hBlockSize, hThreadSize>>>(resizedPaddedImageF4, paddedRegisteredImage, fHOG->rPaddedWidth, fHOG->rPaddedHeight, scale);
	}
