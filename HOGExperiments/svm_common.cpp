...
SVECTOR *create_svector(SVM_WORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (SVM_WORD *)my_malloc(sizeof(SVM_WORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
  vec->twonorm_sq=-1;
...
SVECTOR *create_svector_shallow(SVM_WORD *words,char *userdefined,double factor)
...
SVECTOR *create_svector_n(double *nonsparsevec, long maxfeatnum, char *userdefined, double factor)
{
  ...
  for(i=1;i<=maxfeatnum;i++)  
    if(nonsparsevec[i] != 0) 
      fnum++;
  vec = (SVECTOR *)my_malloc(sizeof(SVECTOR));
  vec->words = (SVM_WORD *)my_malloc(sizeof(SVM_WORD)*(fnum+1));
  fnum=0;
  for(i=1;i<=maxfeatnum;i++) { 
    if(nonsparsevec[i] != 0) {
...
/*MODEL *read_model(char *modelfile) {
    ...
    resp = fread ( &version_buffer, sizeof(char), 10, modelfl );
    qDebug("Version = %s", version_buffer );
    
    if( strcmp( version_buffer, VERSION ) ) {
	qDebug( "Version of model-file does not match version of svm_classify!" ); 
	exit (1); 
    }
    resp = fread( &version, sizeof(int), 1, modelfl );
    
    KERNEL_PARM* kp= &(model->kernel_parm);
    
    resp = fread( &(kp->kernel_type), sizeof(long), 1, modelfl );
    qDebug("Kernel type = %ld", kp->kernel_type );
    resp = fread( &(kp->poly_degree), sizeof(long), 1, modelfl );
    qDebug("Poly degree = %ld", kp->poly_degree );
    resp = fread( &(kp->rbf_gamma), sizeof(double), 1, modelfl );
    qDebug("Gamma = %f", kp->rbf_gamma );
    resp = fread( &(kp->coef_lin), sizeof(double), 1, modelfl ); 
    qDebug("Coef lin = %f", kp->coef_lin );
    resp = fread( &(kp->coef_const), sizeof(double), 1, modelfl );
    qDebug("Coef const = %f", kp->coef_const );
    
    resp = fread( &l, sizeof(long), 1, modelfl );
    qDebug("L = %ld", l );
    resp = fread( &(model->kernel_parm.custom), sizeof(char), l, modelfl );
    qDebug("Param custom = %s", model->kernel_parm.custom );
    resp = fread( &(model->totwords), sizeof(long), 1, modelfl );
    qDebug("Length = %ld", model->totwords );
    resp = fread( &(model->totdoc), sizeof(long), 1, modelfl );
    qDebug("Totdoc = %ld", model->totdoc );
    resp = fread( &(model->sv_num), sizeof(long), 1, modelfl );
    qDebug("Num SV = %ld", model->sv_num );
    resp = fread( &(model->b), sizeof(double), 1, modelfl );
    qDebug("Rho = %f", model->b );
    ...
    words = (SVM_WORD *) my_malloc( sizeof(SVM_WORD) * ( model->totwords + 10 ) );
    qDebug( "Reading weights...");
    for( int i = 1; i < model->sv_num; i++ ) {

	resp = fread( &(model->alpha[i-1]), sizeof( double ), 1, modelfl );
	qDebug("Alpha = %f", model->alpha[i-1] );
	
	for( int j = 0; j < model->totwords; j++ ) {
	    words[j].wnum = j + 1;
	    resp = fread( &(words[j].weight), sizeof( double ), 1, modelfl );
	    printf("[%d] = %f..", words[j].wnum, words[j].weight );
	}
	resp = fread( &l, sizeof(long), 1,modelfl );
	resp = fread( &comment, sizeof(char), l, modelfl );
...
MODEL *read_model(char *modelfile)
{
  ...
  long i,queryid,slackid;
  double costfactor;
  long max_sv, max_words = 0,ll = 0,wpos;
  char *line,*comment;
  SVM_WORD *words;
  ...
  resp = fscanf(modelfl,"SVM-light Version %s\n",version_buffer);
  if(strcmp(version_buffer,VERSION)) {
    perror ("Version of model-file does not match version of svm_classify!"); 
    exit (1); 
  }
  
  resp = fscanf(modelfl,"%ld%*[^\n]\n", &model->kernel_parm.kernel_type);  
  resp = fscanf(modelfl,"%ld%*[^\n]\n", &model->kernel_parm.poly_degree);
  resp = fscanf(modelfl,"%lf%*[^\n]\n", &model->kernel_parm.rbf_gamma);
  resp = fscanf(modelfl,"%lf%*[^\n]\n", &model->kernel_parm.coef_lin);
  resp = fscanf(modelfl,"%lf%*[^\n]\n", &model->kernel_parm.coef_const);
  resp = fscanf(modelfl,"%[^#]%*[^\n]\n", model->kernel_parm.custom);
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (SVM_WORD *)my_malloc(sizeof(SVM_WORD)*(max_words_doc+10));
  if(verbosity>=1) {
...
int parse_document(char *line, SVM_WORD *words, double *label,
