...
__host__ void FastHOG::InitConvolution(int width, int height, bool useGrayscale)
{
	...
	cutilSafeCall( cudaMemcpyToSymbol((char*)d_Kernel, h_Kernel, convKernelSize) );
...
__host__ void FastHOG::SetConvolutionSize(int width, int height)
{
	convWidth = width;
	convHeight = height;
...
__host__ void FastHOG::CloseConvolution()
{
	if (convUseGrayscale)
...
__host__ void FastHOG::ComputeColorGradients1to2(float1* inputImage, float2* outputImage)
{
...
__host__ void FastHOG::ComputeColorGradients4to2(float4* inputImage, float2* outputImage)
{
